            ...
            if (columns.hasOwnProperty(key)) {
                ...
                if (column.editable === true) {
                    ...
                    if (column.hasOwnProperty('custom')) {
                        ...
                        $input.addClass('hidden priority');
                        $td.append(column.custom);
                        $td.find('button[data-role=priority_dropdown]').html((key, svg) => $input.val() + ' ' + svg);
                        $td.find('.priority-dropdown-menu div').prepend(`<button type="button" class="text-gray-700 block w-full px-4 py-2 text-left text-sm hover:bg-primary-lighter hover:text-white" role="menuitem" tabindex="-1" data-value="${$input.val()}" data-role="priority">${$input.val()}</button>`);
                    }
                }
            }
            index ++;
        ...
        $(id).siblings('button[data-role=add]').on('click', function() {
            ...
            for (const key in columns) {
                if (columns.hasOwnProperty(key)) {
                    ...
                    const column = columns[key];
                    if(column.editable === true) {
                        const input = `<input type="${column.type}" class="h-[32px] max-w-[120px] border border-solid border-gray-300 rounded text-center${column.hasOwnProperty('custom') ? ' hidden priority' : ''}">`;
                        $row += input;
                        if (column.hasOwnProperty('custom')) {
                            $row += priorityDropdownMenu;
                        }
